...
export async function run({ page }){
  ...
  return { checks: [
    ...
    ...(await ロックが張りすぎていないか(page)),
  ] };
}

/* ロックの押しの強さ
   ------------------------------------------------------------
   「ロックの音をもう少し抑えてほしい」への歯止め。

   ⚠️ **音量つまみを下げても効かない。** 書き出しは最後に必ずピーク 0.89 へ
      正規化するので、入り口を下げても出口で戻される。実測（同じ曲）:
          つまみ×1.00 → 実効値 0.2009 ／ ×0.75 → 0.1950 ／ ×0.55 → 0.1870
      半分にしても 0.7dB しか下がらない。しかも再生には効いてしまうので、
      「聴いた音と書き出した音がずれない」という約束のほうを壊す。

   効くのは **歪み(drive)** と **他のパートとの差**。どちらも再生でも
   書き出しでも同じように効く。ここではその2つを見張る。 */
async function ロックが張りすぎていないか(page){
  const r = await page.evaluate(() => {
    const S = window.ZCNOVA_ACOUSTIC_SPECS || {};
    const 歪み = { エレキギター:(S.eguitar || {}).drive, キーボード:(S.organ || {}).drive };
    /* 線（58.5dB）よりどれだけ下に置いてあるか。0 なら線と同じ＝抑えていない */
    const 差 = {};
    ['eguitar','organ'].forEach(k => {
      const t = (typeof zcInstTrim === 'function') ? zcInstTrim(k) : null;
      差[k] = t;
    });
    return { 歪み, trim:差,
             ほかの歪み:{ サックス:(S.sax || {}).drive, トランペット:(S.trumpet || {}).drive } };
  });
  const g = r.歪み.エレキギター, o = r.歪み.キーボード;
  return [
    { ok: g > 0 && g <= 0.50 && o > 0 && o <= 0.30,
      label: `ロックの楽器の歪みが張りすぎていない（エレキ ${g} ／ キーボード ${o}）`,
      info: (g <= 0.50 && o <= 0.30)
        ? '入れた当初は エレキ0.62／キーボード0.38 で「もう少し抑えて」と言われた'
        : '音量つまみでは下がらない（書き出しがピーク0.89へ正規化されるため）。下げるのは歪みのほう' },
  ];
}
